...
const web3 = new Web3(Web3.givenProvider  || 'https://rpc.testnet.tomochain.com')
	...
	if (typeof web3 !== 'undefined') {
	    ...
	    async function setupConnected(first=null){
			...
			if(chainid == network){
				...
				// $('.prog-border span').css('margin-left',per+'%');
				...
				if(localStorage.getItem(ADDRESS) != null && localStorage.getItem(ADDRESS).length > 0){
			    	...
			    	$('.connectWallet').html('<button class="button accent-green connected logout ml-2">'+shortText(localStorage.getItem(ADDRESS))+'</button>');
			   	 	...
			   	 	$('.view-link').attr('href','https://testnet.tomoscan.io/address/'+ currentAccount);
			   	}
		    	...
		    	closeApp();
		    	if(first !== true ){
	        		toastr.error('Connect To Smart Chain Network');
				}
		    }
